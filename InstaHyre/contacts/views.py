	...
	def post(self, request):
		...
		if registered_user_qs.exists():
			...
			for contact in personal_contacts_qs:
				...
				contact.save()
			serializer = ContactsSerializer(personal_contacts_qs, many=True)
			return Response(data=serializer.data, status=status.HTTP_200_OK)
			...
			spam_number_obj = Contacts.objects.create(
				phone_number=phone_number,
				spam_count=1
			)
			serializer = ContactsSerializer(spam_number_obj)
...
class UserDetailProfileView(APIView):
	...
	def get(self, request, user_id=None):
		...
		if registered_user_qs.exists():
			...
			contact_connection_check_qs = Contacts.objects.filter(
		...
		contacts_qs = Contacts.objects.filter(id=user_id)
		if contacts_qs.exists():
			contacts_serializer = ContactsSerializer(contacts_qs.first())
